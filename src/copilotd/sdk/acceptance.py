        ...
        if not enabled.get("remoteSteerable"):
            try:
                ...
                try:
                    ...
                    export_safe = not bool(exported.get("remoteSteerable"))
                    self._record(
                        "remote_export_detach_safe",
                        supported=export_safe,
                        status="passed" if export_safe else "export-steerable",
                        detail={"export_steerable": not export_safe},
                    )
            self._record(
                "remote_enable",
                supported=False,
