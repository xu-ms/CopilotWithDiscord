...
from copilotd.ops.contracts import EXPECTED_SDK_VERSION, LATEST_MIGRATION_VERSION
...
def run_operations_audit(repository: Path | None = None) -> OperationsAudit:
    ...
    latest = max((int(name.partition("_")[0]) for name in migrations), default=0)
    checks.append(
        AuditCheck(
            "packaged-migrations",
            latest == LATEST_MIGRATION_VERSION,
            f"latest={latest}, files={len(migrations)}",
        )
    )
