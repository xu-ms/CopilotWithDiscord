...
class SessionLaunchOptions:
            ...
            if transport == "stdio":
                ...
                if not command:
                    raise SessionConfigSnapshotError(
                        f"stdio MCP server {name!r} has no command"
                    )
                ...
                options.update(
                    {
                        ...
                        "env": {
                            reference: variables[reference] for reference in references
                        },
                ...
                if not url:
                    raise SessionConfigSnapshotError(
                        f"http MCP server {name!r} has no URL"
                    )
                options.update(
                    {
                        "type": "http",
    ...
    def sdk_kwargs(self) -> dict[str, Any]:
        ...
        if self.mcp_servers:
            kwargs["mcp_servers"] = {
                name: dict(options) for name, options in self.mcp_servers
            }
