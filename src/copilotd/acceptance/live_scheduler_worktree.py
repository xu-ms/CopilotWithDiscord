        ...
        *,
        ...
        reference = ThreadReference(
            thread_id=str(
                uuid.uuid5(
                    uuid.NAMESPACE_URL,
                    f"copilotd-live:{self.namespace}:{channel_id}:"
                    f"{source_id}:{creation_token}",
                )
            )
        )
...
class LiveSchedulerWorktreeHarness:
    ...
    async def run(self) -> dict[str, Any]:
        ...
        if failures:
            raise LiveAcceptanceError(
                "live acceptance failed: " + ", ".join(failures)
            )
        return summary
    ...
    async def _feature_crash_recovery(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(queue_rows) != 1 or len(submission_rows) != 1:
            raise LiveAcceptanceError(
                "crash recovery did not retain exactly one queue/submission"
            )
        automatic_resubmits = (
            int(queue_after[0])
            - len(queue_rows)
    ...
    async def _feature_blank_worktree(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if binding is None or binding.runtime_remote_mode != "off":
            raise LiveAcceptanceError(
                "blank worktree session did not confirm remote exposure off"
            )
    ...
    async def _feature_history_fork_gate(
        ...
        runtime = (
            None
            if created.thread_id is None
            else context["sessions"].for_thread(created.thread_id)
        )
...
async def _run_crash_child(config_path: Path) -> None:
    ...
    try:
        ...
        definition = await commands.create_message(
            ...
            expression="at:2099-01-01T00:00:00Z",
            text=(
                "Use a shell tool to run `sleep 20`, then reply with exactly "
                f"{token}."
            ),
            timezone="UTC",
