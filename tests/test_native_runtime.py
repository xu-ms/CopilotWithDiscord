...
from copilotd.core.session_runtime import SessionNotReady, SessionRuntime
...
class NativeBridge:
    ...
    async def resume_session(self, _session_id: str, **kwargs: Any) -> NativeHandle:
        self.ingress = kwargs["on_event"]
        return self.handle
...
async def test_native_sdk_events_reconcile_durable_projections(tmp_path: Path) -> None:
    ...
    async with native_runtime(tmp_path, capabilities=supported) as (
        ...
        with pytest.raises(SessionNotReady, match="prior compaction"):
