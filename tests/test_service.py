...
def test_install_supports_local_cli_auth_and_secure_optional_github_token(
    tmp_path: Path,
) -> None:
    local_settings = _settings(tmp_path / "local")
    local_runner = FakeRunner()
    _manager(local_settings, local_runner).install()
    local_payload = json.loads(local_settings.service_secrets_path.read_text(encoding="utf-8"))
    assert local_payload["github_token"] is None

    token_settings = _settings(tmp_path / "token").model_copy(
        update={"github_token": SecretStr("github-test-token")}
    )
    token_runner = FakeRunner()
    token_manager = _manager(token_settings, token_runner)
    token_manager.install()
    token_payload = json.loads(token_settings.service_secrets_path.read_text(encoding="utf-8"))
    assert token_payload["github_token"] == "github-test-token"
    assert b"github-test-token" not in b"".join(token_manager.macos_plists().values())
...
def test_windows_legacy_migration_guard_holds_lock_and_records_handoff(
    ...
    try:
        ...
        connection.close()
    assert versions == set(EXPECTED_MIGRATION_VERSIONS)
    assert handoffs == 1
    script = next(
        call[-1]
        for call in runner.calls
...
@pytest.mark.asyncio
async def test_schema44_restores_unknown_legacy_process_identity_to_null(
    tmp_path: Path,
) -> None:
    settings = _settings(tmp_path)
    now = time.time()
