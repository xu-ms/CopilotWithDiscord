...
@pytest.mark.asyncio
async def test_closed_typed_attachment_only_persists_matching_submission_origin(
    tmp_path: Path,
) -> None:
    async with Database(tmp_path / "typed-origin.sqlite3") as database:
        await database.execute(
            """
            INSERT INTO session_bindings(
                thread_id, project_source, cwd_snapshot, sdk_session_id,
                binding_intent, attachment_state, attachment_reason,
                runtime_generation, owner_fence_token, created_at, updated_at
            ) VALUES ('thread-1', 'explicit', '/closed', 'session-1',
                      'closed', 'attached', 'scheduler_run', 1, 7, 0, 0)
            """
        )
        ...
        await inbox.commit_internal(
            {
                ...
                "data": {
                    **common,
                    "submission_id": "ordinary",
                    "origin": "app_message",
                },
            },
            internal_event_id="typed-origin:ordinary",
        )
        rejected = await database.fetchone(
            "SELECT COUNT(*) FROM message_queue WHERE id = 'ordinary'"
        )
        await inbox.commit_internal(
            {
                "type": "copilotd.submission.queued",
                "data": {
                    **common,
                    "submission_id": "scheduled",
                    "origin": "app_schedule",
                },
            },
            internal_event_id="typed-origin:scheduled",
        )
        accepted = await database.fetchone(
            """
            SELECT submission.origin, queue.state
            FROM submissions AS submission
            JOIN message_queue AS queue ON queue.id = submission.submission_id
            WHERE submission.submission_id = 'scheduled'
            """
        )
        await reducer.stop()
