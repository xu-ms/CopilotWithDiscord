...
| Discord rich gallery v2（历史基线） | passed；run `cd-e2e-1786071615-eb6388be`；94 项自动证据通过；最终现场保留，旧现场已删除 | 该证据验证 typed cards、三个 outbox intent、附件上传、embed-folded CDN image 和主 CDN byte/SHA readback；assistant 正文和 usage/turn summary 后续已回退为 text + compact footer，因此这三项显示形态不再由该历史 rich-card evidence 覆盖；81 项 slash/component 操作为 `pending_human_driver`；Pin message 因 bot 缺少 Manage Messages 阻塞，429 未实际观察 | `0233943194ef2304a6c017bc80e5e3a9ed0331dccb3ce57b520d4703985ec43c` |
