...
| `/project variable list reveal=false` | 项目进程环境变量视图 |
...
| Discord core 命令；strict dynamic builtin manifest；Native-Gated `/ask`、`/session compact`、`/fleet`、`/tasks`、`/agent list|current`、`/after|every list|cancel`、`/remote status|off`、`/review`、`/security-review`、`/research`、`/rubber-duck`；thread-first `/session delete session-id?`；全部 action 由 exact capability 决定 | current-runtime fork 仍 capability-gated 且不注册；`/after|every create` 因 real invoke 返回 `text` 而非 required `completed` 不注册；agent select/deselect、task promote、remote on/export 的 real gate 未通过；elicitation/MCP OAuth 仍待接入 |
...
| tool/subagent/agent-scoped output 归并为原 thread 的单条 TaskDeck；4 秒 cadence、pending coalescing、terminal flush、select/expand/collapse/prev/next；`assistant.reasoning_delta` 仅进入 coalesced thinking status、final reasoning 仅显示 configured concise summary、`assistant.streaming_delta` 不进入 Discord UI；typed task list/show/progress/message/cancel-all/remove/wait 与 Fleet projection；>=8000 字符 tool result/error 逐字附件化；零 child-thread 路径 | real current-promotable fixture 未通过，promote action 不注册；完整 diff artifact lane 尚未实现 |
