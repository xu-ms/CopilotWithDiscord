...
| 17 个 SQLite migration（Foundation `0001`–`0009`，Discord surface `0030`–`0037`；`0010`–`0019` 与 `0020`–`0029` 分别为 Native/Scheduler 的预留 namespace）、project `$HOME` fallback/cwd snapshot、owner fence、creation saga、strict-UUID event journal、reducer-owned operation/submission receipts、submission-task links、liveness leases、startup recovery inventory 与 eager resume | bundled runtime 进程死亡后的 in-flight execution 仍只能标 outcome unknown；experimental task action RPC 继续 gated |
